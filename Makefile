...
###############################################################################
# glslang build_info.h generation
###############################################################################
GLSLANG_BUILD_INFO_H = amalgamation/glslang/glslang/build_info.h
GLSLANG_CHANGES_MD   = amalgamation/glslang/CHANGES.md

# Extract version from CHANGES.md (first line matching ## X.Y.Z)
GLSLANG_VERSION := $(shell grep -m1 -oE '^## [0-9]+\.[0-9]+\.[0-9]+' $(GLSLANG_CHANGES_MD) 2>/dev/null | sed 's/## //')
GLSLANG_VERSION_MAJOR := $(word 1,$(subst ., ,$(GLSLANG_VERSION)))
GLSLANG_VERSION_MINOR := $(word 2,$(subst ., ,$(GLSLANG_VERSION)))
GLSLANG_VERSION_PATCH := $(word 3,$(subst ., ,$(GLSLANG_VERSION)))

$(GLSLANG_BUILD_INFO_H): $(GLSLANG_CHANGES_MD)
	@echo "Generating $@ (version $(GLSLANG_VERSION))"
	@mkdir -p $(dir $@)
	@echo '// Auto-generated by Makefile' > $@
	@echo '#ifndef GLSLANG_BUILD_INFO' >> $@
	@echo '#define GLSLANG_BUILD_INFO' >> $@
	@echo '' >> $@
	@echo '#define GLSLANG_VERSION_MAJOR $(GLSLANG_VERSION_MAJOR)' >> $@
	@echo '#define GLSLANG_VERSION_MINOR $(GLSLANG_VERSION_MINOR)' >> $@
	@echo '#define GLSLANG_VERSION_PATCH $(GLSLANG_VERSION_PATCH)' >> $@
	@echo '#define GLSLANG_VERSION_FLAVOR ""' >> $@
	@echo '' >> $@
	@echo '#define GLSLANG_VERSION_GREATER_THAN(major, minor, patch) \' >> $@
	@echo '    ((GLSLANG_VERSION_MAJOR) > (major) || ((major) == GLSLANG_VERSION_MAJOR && \' >> $@
	@echo '    ((GLSLANG_VERSION_MINOR) > (minor) || ((minor) == GLSLANG_VERSION_MINOR && \' >> $@
	@echo '     (GLSLANG_VERSION_PATCH) > (patch)))))' >> $@
	@echo '' >> $@
	@echo '#define GLSLANG_VERSION_GREATER_OR_EQUAL_TO(major, minor, patch) \' >> $@
	@echo '    ((GLSLANG_VERSION_MAJOR) > (major) || ((major) == GLSLANG_VERSION_MAJOR && \' >> $@
	@echo '    ((GLSLANG_VERSION_MINOR) > (minor) || ((minor) == GLSLANG_VERSION_MINOR && \' >> $@
	@echo '     (GLSLANG_VERSION_PATCH >= (patch))))))' >> $@
	@echo '' >> $@
	@echo '#define GLSLANG_VERSION_LESS_THAN(major, minor, patch) \' >> $@
	@echo '    ((GLSLANG_VERSION_MAJOR) < (major) || ((major) == GLSLANG_VERSION_MAJOR && \' >> $@
	@echo '    ((GLSLANG_VERSION_MINOR) < (minor) || ((minor) == GLSLANG_VERSION_MINOR && \' >> $@
	@echo '     (GLSLANG_VERSION_PATCH) < (patch)))))' >> $@
	@echo '' >> $@
	@echo '#define GLSLANG_VERSION_LESS_OR_EQUAL_TO(major, minor, patch) \' >> $@
	@echo '    ((GLSLANG_VERSION_MAJOR) < (major) || ((major) == GLSLANG_VERSION_MAJOR && \' >> $@
	@echo '    ((GLSLANG_VERSION_MINOR) < (minor) || ((minor) == GLSLANG_VERSION_MINOR && \' >> $@
	@echo '     (GLSLANG_VERSION_PATCH <= (patch))))))' >> $@
	@echo '' >> $@
	@echo '#endif // GLSLANG_BUILD_INFO' >> $@
...
clean:
	rm -rf $(OBJ_DIR) $(LIB_DIR) $(WL_OUT_DIR) $(DL_DIR) $(GLSLANG_BUILD_INFO_H)

###############################################################################
# Library Build
###############################################################################
$(LIB_OUTPUT): $(GLSLANG_BUILD_INFO_H) $(OBJS) | $(LIB_DIR)
...
###############################################################################
glfw_objs: $(OBJ_GLFW)
glsl_objs: $(GLSLANG_BUILD_INFO_H) $(OBJ_GLSL)
cpp_objs:  $(OBJ_CPP)
c_objs:    $(OBJ_C)
...
# Glslang objects need the generated build_info.h header
$(OBJ_DIR)/amalgamation/glslang/%.o: amalgamation/glslang/%.cpp $(DL_ALL) $(GLSLANG_BUILD_INFO_H)
	@mkdir -p $(dir $@)
	$(CXX) $(CXXFLAGS) $(INCLUDEFLAGS) -c $< -o $@
